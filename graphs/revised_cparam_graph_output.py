...
def get_fit(learn_rate, num_hidden_nodes, weight_range, beta, c):
    ...
    block = np.array([1,2,3,4,5,6,7,8])

    #store all the human data in a more convenient way for graphing
    human1 = behavioral_all_structures[0:8,[0]].reshape(8, 1)
    human2 = behavioral_all_structures[0:8,[1]].reshape(8, 1)  

    plt.plot(block, df['shj2'], c='b', marker='*',markersize= 4, linewidth=2,label='shj2 (model)')
    plt.plot(block, human1, c='b',linestyle='dashed', linewidth=3, alpha=0.4,label='shj2 (human)')

    plt.plot(block,df['shj4'], c='y', marker='^',markersize= 4, linewidth=2,label='shj4 (model)')
    plt.plot(block, human2, c='y',linestyle='dashed', linewidth=3, alpha=0.4,label='shj4 (human)')
    

    plt.ylabel('Accuracy')
    plt.xlabel('Learning Block')
    plt.title('DIVA Performance on Revised Ordering SSE='+ str(np.round(fit_err, decimals=3)))
    plt.legend()
    plt.show()
    return fit_err
...
print(get_fit(learn_rate=1.0859924099968392, num_hidden_nodes=10, weight_range=2.9386517301478228, beta=271, c=354))
